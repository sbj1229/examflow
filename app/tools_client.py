...
async def hospital_tools(emit):
    params = StdioServerParameters(command=sys.executable, args=["-m", "app.mcp_server"], env=dict(os.environ))
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            await session.initialize()

            async def call(name, args):
                emit("mcp.call", name, {"arguments": {k:v for k,v in args.items() if k != "session_id"}, "transport": "stdio"})
