...
from app.catalog import ORDERS, POLICY
from app.storage import available_slots, reserve, workspace
...
@mcp.tool()
def reserve_demo_slot(
    session_id: str, order_id: str, slot_id: str, run_id: str, expected_version: int = 1
) -> dict:
    """신뢰된 서버가 승인 후 호출하는 데모 예약. 실제 병원에는 연결되지 않는다."""
    order = ORDERS.get(order_id)
    slot = next((s for s in workspace(session_id)["slots"] if s["id"] == slot_id), None)
    ...
    return reserve(session_id, slot_id, run_id, order_id, expected_version)
