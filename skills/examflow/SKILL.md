...
Antigravity에서 자동 발견하려면 이 `examflow` 디렉터리를 프로젝트의 `.agents/skills/` 아래에 복사한다. 기존 동명 스킬을 덮어쓰지 않는다. 복사된 패키지만 사용하는 경우 위 명령의 skills/examflow/scripts/invoke.py를 이 SKILL.md 옆 scripts/invoke.py의 실제 경로로 바꾼다. 공식 형식 근거: https://antigravity.google/docs/skills
