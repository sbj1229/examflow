...
- 예약 현황·시간표 페이지에서 확정/취소 내역 조회, 예약 취소, 시간 추가·편집·접수 중지를 지원합니다. 변경 내용은 같은 세션의 에이전트 일정 조회에 반영됩니다.
- 예약 중인 시간의 편집과 오래된 제안 승인을 차단하고, 취소 이력의 원래 시간을 보존합니다.
- 에이전트 협업 페이지는 A2A HTTP와 MCP stdio 경로, 호출 수, 구성요소별 실행 근거를 표시합니다. 제품 화면의 반복적인 데모 주의 문구는 제거했습니다.
- Python 25개, 웹 비동기 검사 3개 통과. 로컬 fixture 브라우저에서 시간 편집→제안→확정과 예약 취소→시간 재개방을 확인했습니다.
- 이번 개선은 로컬 구현·검증 단계입니다. 아래 공개 리비전과 기존 발표자료에는 아직 반영하지 않았습니다.
