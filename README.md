...
> 모든 데이터는 합성 사례입니다. 의학적 검사 적합성·진단·실제 예약·메시지 전송은 수행하지 않습니다. 실제 병원의 시간 절감이나 임상 효과를 검증한 서비스가 아닙니다.
...
- 데모의 요청과 A2A Task는 메모리, 예약과 모델 호출 카운터는 인스턴스 로컬 SQLite에 저장합니다. 인스턴스 종료·재배포 후 사라질 수 있습니다. 영구 기록·다중 인스턴스 운영을 제공하지 않습니다.
- 세션당 시간당 20개 요청, 동시 실행 4개, 보관 실행 200개를 제한합니다. 이 제한은 공개 서비스의 완전한 남용 방어나 비용 절대 상한을 보장하지 않습니다.
- 일일 모델 호출 한도는 재시작 시 초기화될 수 있습니다. 실제 운영에서는 외부 영속 카운터·API 인증·예산 모니터링이 필요합니다.
- 모델 형식 오류는 1회 재시도합니다. 원본과 다른 ready나 후보 밖 slot_id는 실패로 처리합니다. 모든 종류의 프롬프트 공격을 방어한다고 주장하지 않습니다.
- 취소는 이후 단계와 확정을 막습니다. 이미 전송한 모델/도구 요청의 계산을 원격 취소하지는 않습니다.
- 준비 상태는 접수 여부이며 임상 적합성이 아닙니다. 실제 적용에는 병원별 업무 규칙 검토와 인증·권한·감사·데이터 보존 설계가 필요합니다.
